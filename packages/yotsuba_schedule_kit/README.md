...
- 三档周 Header、三档课表信息密度、六套配色；课程卡默认使用局部微动态天气材质，图标可选，微光 / 辉光 / 极光 / 呼吸与天气层自动互斥
- 精简 / 适中 / 全面课程详情，课程色 / 天气 / 极简三种 Hero
- 天气快照与 Provider 协议、逐日和小时级预报、课程时段匹配、课程卡局部材质、详情联动和低频模糊天气场景
- 非本周课程保持失色，周六周日与工作日使用相同的小时天气匹配；长课程名与教室信息在紧凑卡片内自动省略
...
`YsWeatherSnapshot.hourly` 接受 `YsHourlyWeather`。课表会按照课程开始节次对应的时间，选择同一天最近的小时点；没有小时数据时才回退到 `daily`。因此上午晴天、下午阵雨的课程可以分别获得对应的卡片材质、动态图标与详情温度。
...
天气场景建议放在应用壳层只渲染一次，让课表、Today、设置和导航共享同一份氛围背景。显式课程光效会替换课程卡天气材质，但不会改变页面级天气场景；非本周课程始终保持失色状态。
